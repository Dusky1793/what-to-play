namespace WhatToPlay.API.Models.SteamApiResponse
{
    ...
    public class SteamGame
    {
        ...
        public string playtime_Forever_Hours => $"{Math.Round((double.Parse(playtime_Forever) / 60), 0).ToString()}";
        public string img_Icon_Url { get; set; }
        public string playtime_Windows_Forever { get; set; }
        public string playtime_Windows_Forever_Hours => $"{Math.Round((double.Parse(playtime_Windows_Forever) / 60), 0).ToString()}";
        public string playtime_Mac_Forever { get; set; }
        public string playtime_Mac_Forever_Hours => $"{Math.Round((double.Parse(playtime_Mac_Forever) / 60), 0).ToString()}";
        public string playtime_Linux_Forever { get; set; }
        public string playtime_Linux_Forever_Hours => $"{Math.Round((double.Parse(playtime_Linux_Forever) / 60), 0).ToString()}";
